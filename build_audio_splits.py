...
INPUT_FILES = [
    DATA_DIR / "ben_shapiro_labeled_audio.jsonl",
    DATA_DIR / "joe_rogan_labeled_audio.jsonl",
    DATA_DIR / "jon_stewart_labeled_audio.jsonl",
]

OUT_DIR = DATA_DIR / "audio_splits"
        ...
        with path.open() as f:
            for line in f:
                ...
                if not line:
                    continue
                rec = json.loads(line)
                if "toxicity" not in rec:
                    continue
                if "audio_path" not in rec:
                    continue
                records.append(rec)
    return records
